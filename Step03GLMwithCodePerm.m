...
outputDir=['D:\Work\dataN\ReplayFMRI\FirstLevelPerm'];
hrf = [0 0 1 5 8 9.2 9 7 4 2 0 -1 -1 -0.8 -0.7 -0.5 -0.3 -0.1 0];
headFF=spm_vol('D:\Work\dataN\Session01\sub01RUN01\bcWGSdswranrun01.nii');
headFF=headFF(1,:);
for subid=1:1:24
    subid
    for SessionId=1:1:2
        ...
        for tid=1:1:8
            ...
            if exist(fpath,'dir')
                ...
                templateAll={};
                for mid=1:1:100
                    lth=length(Onset{mod(mid,15)+1});
                    onsetCC=randperm(length(volsM));
                    onsetCt=onsetCC(lth);
                ...
                for zi=1:1:53
                    for zzi=1:1:63
                        for zzii=1:1:52
                            ...
                            for mmid=1:1:100
                                R=corrcoef(AD,timeSO{mmid});
                                templateAll{mmid}(zi,zzi,zzii)=R(2);
                            end
                        end
                    end
                end
